! function() {
    "use strict";
    angular.module("ion-datetime-picker", ["ionic"]).directive("ionDatetimePicker", function($filter, $ionicLoading, $rootScope, $cordovaToast, $localStorage, UkTimeServices) {
        return {
            restrict: "AE",
            require: "ngModel",
            scope: {
                    ...
                    t.show({
                        ...
                        buttons: [{
                            ...
                            onTap: function() {
                                ...
                                var myDate = new Date();
                                e.commit();
                                if (e.modelDate >= myDate) {
                                    if ($localStorage.checkvalueforcalc == undefined) {
                                        var booktimerdatapro = $filter('date')(e.modelDate, 'dd/MM/yyyy hh:mm:ss');
                                        $rootScope.booktimerdatapro = booktimerdatapro;
                                        $localStorage.datetimeselected = $filter('date')(new Date(e.modelDate), 'yyyy-MM-dd hh:mm:ss');
                                        console.info($localStorage.datetimeselected);
                                    }
                                    if ($localStorage.checkvalueforcalc != undefined) {
                                        var booktimerdata = $filter('date')(e.modelDate, 'dd/MM/yyyy hh:mm:ss');
                                        $rootScope.booktimer = booktimerdata;
                                        console.info($rootScope.booktimer)
                                        UkTimeServices.sendresdule().then(function(_response) {
                                            $ionicLoading.hide();
                                            console.info(_response.success);
                                    ...
                                    $cordovaToast.showLongCenter('You Selected Past time');
                                    e.showPopup();
                                    return false;
                                }
                            }
